...
describe("Teste - Login", () => {
  ...
  .then(($input) => {
    ...
    input.focus();
    input.value = 'Vitrine Portugues Cypress';
...
cy.get('input[placeholder="Título"]')
  .then(($input) => {
    ...
    input.removeAttribute('disabled');
    input.focus();
    input.value = 'Título Automação Cypress PORTUGUES';
...
cy.get('input[placeholder="Descrição"]')
  .then(($input) => {
    ...
    input.removeAttribute('disabled');
    input.focus();
    input.value = 'Descrição Automação Cypress PORTUGUES';
...
cy.get('a.banner-container', { timeout: 10000 })
  ...
  const tituloVitrine = 'Título Automação Cypress PORTUGUES';
const descricaoVitrine = 'Descrição Automação Cypress PORTUGUES';
